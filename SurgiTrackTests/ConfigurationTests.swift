...
struct ConfigurationTests {
    ...
    @Test func environment_current_returnsValidEnvironment() async throws {
        let env = AppConfiguration.Environment.current

        // Should be one of the valid environments
        let validEnvs: [AppConfiguration.Environment] = [.development, .staging, .production]
    ...
    @Test func security_sessionTimeout_isPositive() async throws {
        let timeout = AppConfiguration.Security.sessionTimeout
    ...
    @Test func security_pinLength_isValid() async throws {
        let pinLength = AppConfiguration.Security.pinLength
    ...
    @Test func security_minPasswordLength_isSecure() async throws {
        let minLength = AppConfiguration.Security.minPasswordLength
    ...
    @Test func features_auditLogging_isEnabled() async throws {
        let enabled = AppConfiguration.Features.auditLoggingEnabled
    ...
    @Test func app_buildNumber_isNotEmpty() async throws {
        let build = AppConfiguration.App.buildNumber
    ...
    @Test func app_fullVersion_combinesVersionAndBuild() async throws {
        let version = AppConfiguration.App.version
        let build = AppConfiguration.App.buildNumber
        let fullVersion = AppConfiguration.App.fullVersion
    ...
    @Test func app_bundleIdentifier_isNotEmpty() async throws {
        let bundleId = AppConfiguration.App.bundleIdentifier
    ...
    @Test func storage_imageCompression_isValid() async throws {
        let quality = AppConfiguration.Storage.imageCompressionQuality
    ...
    @Test func storage_maxAttachmentSize_isLargerThanImage() async throws {
        let maxAttachment = AppConfiguration.Storage.maxAttachmentSize
        let maxImage = AppConfiguration.Storage.maxImageSize
    ...
    @Test func ui_animationDuration_isReasonable() async throws {
        let duration = AppConfiguration.UI.animationDuration
    ...
    @Test func ui_toastDuration_isReadable() async throws {
        let duration = AppConfiguration.UI.toastDuration
    ...
    @Test func ui_searchDebounce_isResponsive() async throws {
        let delay = AppConfiguration.UI.searchDebounceDelay
    ...
    @Test func ui_pageSize_isReasonable() async throws {
        let pageSize = AppConfiguration.UI.pageSize
    ...
    @Test func environmentVariable_nonExistent_returnsNil() async throws {
        let result = AppConfiguration.environmentVariable("DEFINITELY_NOT_A_REAL_VAR_12345")
    ...
    @Test func validate_inDevelopment_succeeds() async throws {
        ...
        #if DEBUG
        let isValid = AppConfiguration.validate()
